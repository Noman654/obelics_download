...
PATH_LINE_DEDUP_DOMAIN_TO_POSITIONS_LOCAL = f"{root_dir}/scratch/line_dedup_domain_to_positions.json"

PATH_SAVE_S3_LINE_DEDUP_DOMAIN_TO_POSITIONS_SHARDED = (
    "s3://llm-spark/multi_modal/commoncrawl/webdocs/line_dedup_domain_to_positions_sharded/"
)
...
if __name__ == "__main__":
    ...
    for idx_shard in tqdm(range(NUM_SHARDS)):
        ...
        with open(f"{root_dir}/scratch/line_dedup_domain_to_positions_{idx_shard}.json", "w") as f:
            json.dump(domain_to_positions_shard[idx_shard], f)

    for idx_shard in tqdm(range(NUM_SHARDS)):
        path_disk = f"{root_dir}/scratch/line_dedup_domain_to_positions_{idx_shard}.json"
